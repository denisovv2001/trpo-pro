...
if($_POST['age_game'] == "3-5") // поиск фильтра возраст от 3 до 5
{
    //////////////////////////////
    if($_POST['min_player'] >= 1 and $_POST['min_time'] >= 1){ //если выбрано кол-во игроков и время игры
        ...
        $result = $mysql->query("SELECT * FROM `game ad`  WHERE `sity` = '$city' AND `age` <= 5 AND `quality_max` <= '$max_player' AND `max_time` <= '$max_time'");
        ...
        if($result->num_rows > 0){
            ...
            Возраст: <span class="badge bg-white text-primary">от 3 - 5 лет </span>
            </button>
            <button type="button" class="btn btn-primary mb-2 me-2">
    ...
    if($_POST['min_player'] >= 1) //если выбранно кол-во играков
    {
        ...
        $result = $mysql->query("SELECT * FROM `game ad`  WHERE `sity` = '$city' AND `age` <= 5 AND `quality_max` <= '$max'");
   ...
   ///////////////////////////////
    if($_POST['min_time'] >= 1){ //если выбрано время игры
        ...
        $result = $mysql->query("SELECT * FROM `game ad`  WHERE `sity` = '$city' AND `age` <= 5 AND `max_time` <= '$max'");
...
/////////////////////////////
    if($_POST['age_game'] == "3-5" and empty($_POST['min_player']) and empty($_POST['min_time'])) {    //если не выбирал кол-во игрок и время игры 
        $city = $_POST['city_choice'];
       
        $result = $mysql->query("SELECT * FROM `game ad` WHERE `sity` = '$city' AND `age` <= 5 ");
        if($result->num_rows > 0){
            echo '
        <div class="card-body mt-3  px-0">
        <h4 class="text-primary px-0">Фильтры:</h4>
        <button type="button" class="btn btn-primary mb-2">
        Возраст  <span class="badge bg-white text-primary"> от 3 - 5 </span>
        </button>
        <button type="button" class="btn btn-primary mb-2">
        Город  <span class="badge bg-white text-primary">'.$city.'</span>
        </button>
